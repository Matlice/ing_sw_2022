...
\end{frame}
\note{
    La funzione presente nel Controller e richiamata nella procedura di creazione gerarchia.

    Vengono salvate le categorie come LeafCategory e solo se necessitano l'aggiunta dei figli esse vengono convertite in NodeCategory.

    NON SEGUE SOLUZIONE, SEGUE ALTRA VIOLAZIONE
}
...
\end{frame}
\note{
    La funzione è definita in Category ma dovrebbe essere differenziata per i due tipi figli
}
...
\end{frame}
\note{
    La chiamata a convertToNode() viene definita per le due tipologie nel seguente modo:

    Se LeafCategory: Viene effettuata la conversione effettiva

    Se NodeCategory: Viene ritornata la categoria senza modifiche.
}
