...
\begin{frame}
    ...
    \begin{figure}
        \centering
        \plantumlfile[width=.60\textwidth]{uml/mvc_classi.puml}
...
\begin{frame}{Scambio dei dati tra gli strati}
    ...
    \begin{minipage}{.49\textwidth}
        \begin{figure}
            \centering
            \plantumlfile[width=.85\textwidth]{uml/mvc.ssd.puml}
...
\end{frame}
\note{
    Come è possibile vedere, non vi è mai una comunicazione diertta tra View e Model, ma il controller sarà sempre intermediario e facciata tra i vari metodi. In particolare il compito effettivo del Controller sarà di raccogliere e fornire i dati al model per effettuare le operazioni, oltre che a gestire la logica utente.
}
...
\begin{frame}
    ...
    \begin{minipage}{.49\textwidth}
        \begin{figure}
            \centering
            \plantumlfile[width=.7\textwidth]{uml/mvc.menu-select.puml}
            \caption{Diagramma di attività di utilizzo menu e scelta utente}
        \end{figure}
    \end{minipage}
\end{frame}
\note{
    A seguire spiegazione del funzionamento del menu.
}
...
\end{frame}
\note{
    Il funzionamento del menu consiste nel proporre una scelta da una lista di MenuEntry, le quali hanno associato un testo stampabile, dati di visualizzazione (disabled, index...) e un'azione eseguibile.
    L'azione eseguita dalla MenuEntry derivata da una MenuEntryWrapper consiste nel ritornare l'oggetto originale di tipo MenuAction<T>.
}
...
\end{frame}
\note{
    view.DataTypeConverter è la factory dinamica che viene istanziata nel momento in cui la StreamView viene definita, il quale compito consiste nel creare oggetti adattatori per la conversione di classi Modello

    Ogni tipo di classe appartenente al model viene registrato con un relativo cnverter, che si occuperà della creazione effettiva dell'adapter in funzione dell'oggetto.
